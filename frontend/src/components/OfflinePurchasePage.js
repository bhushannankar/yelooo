...
const OfflinePurchasePage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [sellersRes, subsRes] = await Promise.all([
        axios.get(`${API_URL}/OfflineTransaction/sellers`, { headers: getAuthHeader() }),
        axios.get(`${API_URL}/OfflineTransaction/my-submissions`, { headers: getAuthHeader() })
      ]);
      setSellers(Array.isArray(sellersRes.data) ? sellersRes.data : sellersRes.data?.$values || []);
      setSubmissions(Array.isArray(subsRes.data) ? subsRes.data : subsRes.data?.$values || []);
      setError(null);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const sellerId = parseInt(form.sellerId, 10);
    const amount = parseFloat(form.amount);
    if (!sellerId || amount <= 0) {
      setError('Please select a seller and enter a valid amount.');
      return;
    }
  ...
  return (
    <div className="offline-purchase-wrapper">
      ...
      <div className="offline-purchase-container">
        <h1>Submit Offline Purchase</h1>
        <p className="page-subtitle">Purchased from a Yelooo seller at their store? Upload your bill to earn points after admin verification.</p>
        ...
        <div className="my-submissions-section">
          ...
          {loading ? (
            ...
            <div className="submissions-table-wrap">
              <table>
                <thead>
                  <tr><th>Date</th><th>Seller</th><th>Amount</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {submissions.map((s) => (
...
export default OfflinePurchasePage;
